...
class ReplayBuffer:
    def __init__(self, obs_dim, act_dim, book_dim, size, batch_size=32):
        self.obs1_buf = np.zeros([size, obs_dim], dtype=np.float32)
        self.obs2_buf = np.zeros([size, obs_dim], dtype=np.float32)
        self.acts_buf = np.zeros([size, act_dim], dtype=np.float32)
        self.books1_buf = np.zeros([size, book_dim], dtype=np.float32)
        self.books2_buf = np.zeros([size, book_dim], dtype=np.float32)
        self.rews_buf = np.zeros(size, dtype=np.float32)
        self.done_buf = np.zeros(size, dtype=np.float32)
        self.ptr, self.size, self.max_size = 0, 0, size
        self.batch_size = batch_size

    def store(self, obs, act, book, rew, next_obs, next_book, done):
        self.obs1_buf[self.ptr] = obs
        self.obs2_buf[self.ptr] = next_obs
        self.acts_buf[self.ptr] = act
        self.books1_buf[self.ptr] = book
        self.books2_buf[self.ptr] = next_book
...
def create_critic(num_states, num_actions, num_books):
    ...
    input_book = tf.keras.layers.Input(shape=[num_books])
    concate1 = tf.keras.layers.concatenate([input_state, input_action, input_book])
    h1 = tf.keras.layers.Dense(128, activation='relu')(concate1)
    concate2 = tf.keras.layers.concatenate([h1, input_action])
    h2 = tf.keras.layers.Dense(128, activation='relu')(concate2)
    q = tf.squeeze(tf.keras.layers.Dense(1, activation=None)(h2), axis=1)
    model = tf.keras.Model(inputs=[input_state, input_action, input_book], outputs=[q])
...
class TD3:
    def __init__(self, n_states_1h, n_actions_1h, n_books, tau, q_lr_1h, mu_lr_1h, gamma, batch_size,
        ...
        self.memory = ReplayBuffer(n_states_1h, n_actions_1h, n_books, replay_capacity, batch_size)
        ...
        self.q_1h, self.t_q_1h, self.q2_1h, self.t_q2_1h = create_critic(n_states_1h, n_actions_1h, n_books)

    @tf.function
    def tf_train_critic_1h_step(self, states, actions, books, rewards, next_states, next_books, dones):
        ...
        next_q_values = self.t_q_1h((next_states, next_mu_noise, next_books))
        next_q2_values = self.t_q2_1h((next_states, next_mu_noise, next_books))
        ...
        with tf.GradientTape(persistent=True) as tape:
            q1 = self.q_1h((states, actions, books))
            q2 = self.q2_1h((states, actions, books))
    ...
    def train_critic_1h(self):
        ...
        states, actions, books, rewards, next_states, next_books, dones = self.experience
        q_loss = self.tf_train_critic_1h_step(states, actions, books, rewards, next_states, next_books, dones)
        return q_loss

    @tf.function
    def tf_train_actor_1h_step(self, states, books):

        with tf.GradientTape() as tape:
            mu = self.mu_1h(states)
            q_mu = self.q_1h((states, mu, books))
    ...
    def train_actor_1h(self):
        states, actions, books, rewards, next_states, next_books, dones = self.experience
        mu_loss = self.tf_train_actor_1h_step(states, books)
        return mu_loss
